...
public class LoginApp {
    ...
    private final Map<Integer, List<NewEquip>> newEquip;
    ...
    public boolean checkCharEquip(int raceSelect, int gender, int type, int itemID) {
        List<NewEquip> newEquipList = newEquip.getOrDefault(raceSelect, new ArrayList<>());
        for (NewEquip equip : newEquipList) {
    ...
    private void createAcceptor() {
        this.acceptor = new LoginAcceptor(new InetSocketAddress(port));
        this.acceptor.start();
    }
    ...
    private void loadNewCharInfo() {
        ...
        if (etcDir != null) {
            ...
            if (makeChar != null) {
                WzProperty normal = makeChar.getNode("Info");
                if (normal != null) {
                    List<NewEquip> newEquipList = new ArrayList<>();
                    for (WzProperty makeGender : normal.getChildNodes()) {
                        ...
                        for (WzProperty makeType : makeGender.getChildNodes()) {
                            for (WzProperty makeInfo : makeType.getChildNodes()) {
                                newEquipList.add(new NewEquip(gender, Byte.parseByte(makeType.getNodeName()), WzUtil.getInt32(makeInfo, 0)));
                            }
                        }
                    }
                    newEquip.put(RaceSelect.NORMAL, newEquipList);
                }

                List<NewEquip> newEquipList = new ArrayList<>();
                for (int i = 0; i < 2; i++) {
                    for (WzProperty makeType : makeChar.getNode(i == 0 ? "PremiumCharMale" : "PremiumCharFemale").getChildNodes()) {
                        for (WzProperty makeInfo : makeType.getChildNodes()) {
                            newEquipList.add(new NewEquip(i, Byte.parseByte(makeType.getNodeName()), WzUtil.getInt32(makeInfo, 0)));
                        }
                    }
                }
                newEquip.put(RaceSelect.CYGNUS, newEquipList);

                newEquipList = new ArrayList<>();
                for (int i = 0; i < 2; i++) {
                    for (WzProperty makeType : makeChar.getNode(i == 0 ? "OrientCharMale" : "OrientCharFemale").getChildNodes()) {
                        for (WzProperty makeInfo : makeType.getChildNodes()) {
                            newEquipList.add(new NewEquip(i, Byte.parseByte(makeType.getNodeName()), WzUtil.getInt32(makeInfo, 0)));
                        }
                    }
                }
                newEquip.put(RaceSelect.ARAN, newEquipList);

                newEquipList = new ArrayList<>();
                for (int i = 0; i < 2; i++) {
                    for (WzProperty makeType : makeChar.getNode(i == 0 ? "EvanCharMale" : "EvanCharFemale").getChildNodes()) {
                        for (WzProperty makeInfo : makeType.getChildNodes()) {
                            newEquipList.add(new NewEquip(i, Byte.parseByte(makeType.getNodeName()), WzUtil.getInt32(makeInfo, 0)));
                        }
                    }
                }
                newEquip.put(RaceSelect.EVAN, newEquipList);

                newEquipList = new ArrayList<>();
                for (int i = 0; i < 2; i++) {
                    for (WzProperty makeType : makeChar.getNode(i == 0 ? "ResistanceCharMale" : "ResistanceCharFemale").getChildNodes()) {
                        for (WzProperty makeInfo : makeType.getChildNodes()) {
                            newEquipList.add(new NewEquip(i, Byte.parseByte(makeType.getNodeName()), WzUtil.getInt32(makeInfo, 0)));
                        }
                    }
                }
                newEquip.put(RaceSelect.RESISTANCE, newEquipList);
            }
            etcDir.release();
        }
        etcDir = null;
    ...
    public void start() {
        try {
            Logger.logReport("MapleStory GL Service WvsLogin.exe.");
            setUp();
