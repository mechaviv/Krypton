...
import common.login.LoginResCode;
import common.login.LoginState;
import common.login.RaceSelect;
...
public class ClientSocket extends SimpleChannelInboundHandler {
    ...
    private boolean isVACDlgOn;

    public ClientSocket(Channel socket) {
        this.channel = socket;
        this.lockSend = new ReentrantLock();
        this.loginState = LoginState.WaitCheckPassword;
        this.failCount = 0;
        this.accountID = 0;
        this.worldID = -1;
        this.channelID = -1;
    ...
    public void onClose() {
        ...
        if (this.loginState < LoginState.WaitCenterSelectWorldResult || this.loginState >= LoginState.Completed)
            this.accountID = 0;
        this.loginState = LoginState.Disconnected;
    ...
    public void onCheckDuplicateID(InPacket packet) {
        if (this.loginState == LoginState.WaitSelectCharacter) {
    ...
    public void onCheckPassword(InPacket packet) {
        if (this.loginState != LoginState.WaitCheckPassword) {
            onClose();
            return;
        }
        ...
        if (retCode == LoginResCode.Success) {
    ...
    public void onWorldRequest(InPacket packet) {
        if (true) {// checks if pin code checked or admin client and with correct ip for admin
            sendWorldInformation();
            loginState = LoginState.WaitSelectWorld;
        } else {
            Logger.logError("'User tried to select world without PinCode Check.");
            postClose();
        }
    }

    public void onWorldInfoRequest(InPacket packet) {
        if (loginState == LoginState.WaitCheckPassword || isVACDlgOn) {
            sendWorldInformation();
        } else {
            postClose();
        }
    ...
    public void onLogoutWorld(InPacket packet) {
        // this->m_bIsPinCodeChecked = 0;
        loginState = LoginState.WaitCheckPassword;
        LoginAcceptor.getInstance().removeSocket(this);// not sure about this
    }

    public void onCheckUserLimit(InPacket packet) {
        int worldID = packet.decodeShort();
        ...
        if (entry != null) {
            sendPacket(LoginPacket.onCheckUserLimit(), false);
        } else {
            sendPacket(LoginPacket.onSelectWorldResult(LoginResCode.NotConnectableWorld, 0, null), false);
        }
    }
    ...
    public void onCreateNewCharacter(InPacket packet) {
        ...
        int selectedRace = packet.decodeInt();
        int selectedSubJob = packet.decodeShort();

        int face = packet.decodeInt();
        int hairStyle = packet.decodeInt();
        int hairColor = packet.decodeInt();
        int skinColor = packet.decodeInt();
        ...
        if (!LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.Face, face)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.HairStyle, hairStyle)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.HairColor, hairColor)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.SkinColor, skinColor)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.Clothes, clothes)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.Pants, pants)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.Shoes, shoes)
                || !LoginApp.getInstance().checkCharEquip(selectedRace, gender, NewCharEquipType.Weapon, weapon))
            ret = false;
        ...
        if (ret && world != null && world.getSocket() != null) {
            ...
            int result = LoginDB.rawCreateNewCharacter(this.accountID, this.worldID, charName, this.gender, face, 0, hairStyle, level, job, selectedSubJob, clothes, pants, shoes, weapon, stat, map, newCharacterID);
            if (result == LoginResCode.Success) {
    ...
    public void onExceptionLog(InPacket packet) {
        ...
        Logger.logError("[EXCEPTION] %s", log);
    }
    ...
    public void onSelectCharacter(InPacket packet) {
        if (this.loginState == LoginState.WaitSelectCharacter) {
            ...
            if (world != null) {
                ...
                if (ch != null) {
                    this.loginState = LoginState.WaitCenterSelectCharacterResult;
                    sendPacket(LoginPacket.onSelectCharacterResult(LoginResCode.Success, Utilities.netIPToInt32(ch.getAddr()), ch.getPort(), this.characterID), false);
                }
            }
        } else {
            postClose();
    ...
    public void onSelectWorld(InPacket packet) {
        ...
        if (pWorld != null) {
            ...
            this.loginState = LoginState.WaitSelectCharacter;
            sendPacket(LoginPacket.onSelectWorldResult(LoginResCode.Success, this.ssn, avatars), false);
        } else {
        ...
        //} else if (type == ClientPacket.AliveReq) {
            ...
            switch (type) {
                case ClientPacket.CheckPassword:
                    ...
                    break;
                case ClientPacket.CheckPinCode:
                    break;
                case ClientPacket.UpdatePinCode:
                    break;
                case ClientPacket.WorldRequest:
                    onWorldRequest(packet);
                    break;
                case ClientPacket.WorldInfoRequest:
                    onWorldInfoRequest(packet);
                    break;
                case ClientPacket.LogoutWorld:
                    onLogoutWorld(packet);
                    break;
                case ClientPacket.CheckUserLimit:
                    onCheckUserLimit(packet);
                    break;
                case ClientPacket.SelectWorld:
                    onSelectWorld(packet);
                    break;
                case ClientPacket.EnableSPWRequest:
                    // CClientSocket::OnEnableSPW(iPacket);
                    break;
                case ClientPacket.CheckSPWRequest:
                    // CClientSocket::OnCheckSPW(iPacket);
                    break;
                case ClientPacket.EnableSPWRequestByACV:
                    // CClientSocket::OnEnableSPWByACV(iPacket);
                    break;
                case ClientPacket.CheckSPWRequestByACV:// by all char view
                    // CClientSocket::OnCheckSPWByACV(iPacket);
                    break;
                ...
                case ClientPacket.SelectCharacter:
                    ...
                    break;
                case ClientPacket.ViewAllChar:
                    break;
                case ClientPacket.SelectCharacterByVAC:
                    break;
                case ClientPacket.VACFlagSet:
                    break;
    ...
    public void sendWorldInformation() {
        for (WorldEntry world : LoginApp.getInstance().getWorlds()) {
            sendPacket(LoginPacket.onWorldInformation(world), false);
        }
        sendPacket(LoginPacket.onWorldInformation(null), false);
    }
