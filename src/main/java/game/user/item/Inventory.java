...
public class Inventory {
    ...
    public static int tryExchange(User user, int incMoney, List<ActItem> actItems) {
        if (incMoney == 0 && (actItems == null || actItems.size() <= 0)) {
            return QuestFlag.QuestRes_Act_Success;
        }
        List<ExchangeElem> exchangeElems = new ArrayList<>();
        ...
        for (ActItem actItem : actItems) {
            int itemID = actItem.getInfo().getItemID();
            int count = actItem.getInfo().getCount();
            System.out.println("Item ID [1] " + itemID);
            System.out.println("Count [1] " + count);
            if (count == 0) {
                count = -getItemCount(user, itemID);
            }
            if ((ItemInfo.isQuestItem(itemID) || ItemInfo.isOnlyItem(itemID)) && count > 0) {
                if (ItemInfo.getEquipItem(itemID) != null) {
                    Pointer<Integer> bodyPart = new Pointer<>(0);
                    ItemAccessor.getBodyPartFromItem(itemID, user.getCharacter().getCharacterStat().getGender(), bodyPart, false);
                    ItemSlotBase item = user.getCharacter().getItem(ItemType.Equip, -bodyPart.get());
                    if (item != null && item.getItemID() == itemID) {
                        if (ItemInfo.isOnlyItem(itemID)) {
                            return QuestFlag.QuestRes_Act_Failed_OnlyItem;
                        }
                        continue;
                    }
                }
            ...
            int job = user.getCharacter().getCharacterStat().getJob();
            // bms = ((1 << job / 100) & option.getJobFlag()) != 0
            Logger.logReport("Job Bit Flag Bool = [%s]", (JobAccessor.getJobBitflag(job) & option.getJobFlag()) != 0);
            Logger.logReport("Grade Code Bool = [%s]", (user.getGradeCode() >= UserGradeCode.GM.getGrade() && JobAccessor.getJobCategory(job) == JobCategory.ADMIN));
            if ((JobAccessor.getJobBitflag(job) & option.getJobFlag()) != 0 || (user.getGradeCode() >= UserGradeCode.GM.getGrade() && JobAccessor.getJobCategory(job) == JobCategory.ADMIN)) {
                ExchangeElem element = new ExchangeElem();
                if (count <= 0) {
                    element.r.itemID = itemID;
                    element.add = false;
                    element.r.count = (short) -count;
                    element.r.ti = 0;
                    element.r.pos = 0;
                } else {
                    if (ItemInfo.isCashItem(itemID)) {
                        Logger.logError("CashItem is in a Quest ActItem - Item : %d", itemID);
                        return QuestFlag.QuestRes_Act_Failed_Unknown;
                    }
                    ItemSlotBase item = ItemInfo.getItemSlot(itemID, option.getVariation());
                    if (item == null) {
                        return QuestFlag.QuestRes_Act_Failed_Unknown;
                    }
                    item.setItemNumber(count);
                    if (option.isNamed()) {
                        // todo naming item to character's name
                    }
                    if (option.getPeriod() != 0) {
                        item.setDateExpire(FileTime.getDateExpireFromPeriod(option.getPeriod()));
                    }
                    if (option.getDateExpire() != null) {
                        if (FileTime.compareFileTime(option.getDateExpire(), FileTime.DATE_2079) != 0) {
                            item.setDateExpire(option.getDateExpire());
                        }
                    }
                    if (!ItemInfo.isValidTimeLimitedItem(item)) {
                        return QuestFlag.QuestRes_Act_Failed_Unknown;
                    }
                    FileTime curFt = FileTime.systemTimeToFileTime();
                    if (item != null) {
                        if (FileTime.compareFileTime(item.getDateExpire(), curFt) <= 0) {
                            return QuestFlag.QuestRes_Act_Failed_Unknown;
                        }
                        element.initAdd(0, (short) 0, item);
                    }
                }
                exchangeElems.add(element);
                items.add(itemID);
                itemsCount.add(count);
            }
        }
        if (exchange(user, incMoney, exchangeElems, logAdd, logRemove)) {
            sendInventoryOperation(user, Request.None, logRemove);
            sendInventoryOperation(user, Request.None, logAdd);
            if (items.size() > 0 && itemsCount.size() == items.size()) {
                user.postQuestEffect(true, items, itemsCount, null, 0);
            }
            if (incMoney != 0) {
                user.sendIncMoneyMessage(incMoney);
            }
            return QuestFlag.QuestRes_Act_Success;
        }
        ItemSlotBase item;
        int index = 0;
        while (true) {
            if (exchangeElems == null || index >= exchangeElems.size()) {
                return QuestFlag.QuestRes_Act_Failed_Inventory;
            }
            ExchangeElem exchangeElem = exchangeElems.get(index);
            int itemID = exchangeElem.r.itemID;
            if (exchangeElem.add || itemID == 0 || ItemInfo.getEquipItem(itemID) == null) {
                index++;
                continue;
            }
            Pointer<Integer> bodyPart = new Pointer<>(0);
            ItemAccessor.getBodyPartFromItem(itemID, user.getCharacter().getCharacterStat().getGender(), bodyPart, false);
            item = user.getCharacter().getItem(ItemType.Equip, -bodyPart.get());
            if (item == null || itemID != item.getItemID()) {
                continue;
            }
            break;
        }
        return QuestFlag.QuestRes_Act_Failed_Equipped;
    }
}
