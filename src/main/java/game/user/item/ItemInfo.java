...
public class ItemInfo {
    ...
    public static BundleItem getBundleItem(int itemID) {
        return bundleItem.getOrDefault(itemID, null);
    }

    public static EquipItem getEquipItem(int itemID) {
        return equipItem.getOrDefault(itemID, null);
    }
    ...
    public static String getItemName(int itemID) {
        ...
        return value.getOrDefault("name", "NULL");
    }
    ...
    public static boolean isOnlyItem(int itemID) {
        if (ItemAccessor.getItemTypeIndexFromID(itemID) == ItemType.Equip) {
            EquipItem equip = getEquipItem(itemID);
            if (equip != null) {
                return equip.isOnly();
    ...
    public static boolean isValidTimeLimitedItem(ItemSlotBase item) {
        if (FileTime.compareFileTime(item.getDateExpire(), FileTime.DATE_2079) < 0) {
            BundleItem bundleItem = getBundleItem(item.getItemID());
            if (bundleItem != null) {
                return bundleItem.isTimeLimited();
            } else {
                EquipItem equipItem = getEquipItem(item.getItemID());
                if (equipItem == null) {
                    Logger.logError("Invalid Script Item Exchange Data TimeLimited (Item ID: %d)", item.getItemID());
                    return false;
                }
                return equipItem.isTimeLimited();
            }
        }
        return true;
    }
    ...
    public static void load() {
        Logger.logReport("Loading Item String");
        iterateItemString();
        iterateMapString();
    ...
    private static void iterateItemString() {
        WzPackage stringDir = new WzFileSystem().init("String").getPackage();
        if (stringDir != null) {
            String[] cat = {"Eqp.img", "Consume.img", "Ins.img", "Etc.img", "Cash.img"};
            for (String c : cat) {
                WzProperty category = stringDir.getItem(c);
                if (c.contains("Eqp")) {
                    category = category.getNode("Eqp");
                }
                for (WzProperty item : category.getChildNodes()) {
                    if (c.contains("Eqp") || c.contains("Etc")) {
                        for (WzProperty item2 : item.getChildNodes()) {
                            int itemID = Integer.parseInt(item2.getNodeName());
                            Map<String, String> strings = new HashMap<>();
                            for (WzProperty data : item2.getChildNodes()) {
                                strings.put(data.getNodeName(), WzUtil.getString(data, ""));
                            }
                            itemString.put(itemID, strings);
                        }
                    } else {
                        int itemID = Integer.parseInt(item.getNodeName());
                        Map<String, String> strings = new HashMap<>();
                        for (WzProperty data : item.getChildNodes()) {
                            strings.put(data.getNodeName(), WzUtil.getString(data, ""));
                        }
                        itemString.put(itemID, strings);
                    }
    ...
    private static void registerEquipItemInfo(WzProperty itemData) {
        ...
        if (info != null) {
            item.setItemName(getItemName(item.getItemID()));
