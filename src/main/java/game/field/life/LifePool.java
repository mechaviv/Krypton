...
public class LifePool {
    ...
    public boolean createMob(int templateID, MobGen pmg, int x, int y, short fh, boolean noDropPriority, int type, int option, byte left, int mobType, Controller owner) {
        ...
        if (owner != null && owner.getCtrlCount() >= 50) {
            if (type != MobAppearType.REVIVED || mobType != 2 || !giveUpMobController(owner)) {
                owner = null;
            }
        }
        ...
        if (template != null && owner != null) {
            ...
            if (type != MobAppearType.SUSPENDED) {
                mob.setSummonType(MobAppearType.NORMAL);
                mob.setSummonOption(0);
            }
    ...
    public void removeMobByItem(int mobTemplateID, User user) {
        for (Mob mob : mobs.values()) {
            int distanceX = mob.getCurrentPos().x - user.getCurrentPosition().x;
    ...
    public void update(long time) {
        ...
        for (Mob mob : mobs.values()) {
            mob.update(time);
            if (mob.isTimeToRemove(time, mob.getTemplateID() == 9999999)) {
                FieldSplit fieldSplit = getField().splitFromPoint(mob.getCurrentPos().x, mob.getCurrentPos().y);
                if (fieldSplit != null) {
                    FieldSplit[] splits = new FieldSplit[9];
                    getField().getEncloseSplit(fieldSplit, splits);
                    for (int i = 0; i <splits.length; i++) {
                        FieldSplit split = splits[i];
                        if (split == null) {
                            continue;
                        }
                ...
                if (mob.getTemplateID() == 9999999) {
                    Controller controller = mob.getController();
                    if (controller != null && controller.getUser() != null) {
                        int userID = controller.getUser().getCharacterID();
                        //CLifePool::RemoveFixedMob(pController->pUser->m_dwCharacterID);
                        //CLifePool::CreateFixedMob(pController->pUser->m_dwCharacterID, pController);
                    } else {
                        Logger.logError("Fixed Mob - No Controller");
                    }
                } else {
                    removeMob(mob);
                }
                    ...
                    for (Iterator<AttackInfo> it = attack.iterator(); it.hasNext();) {
                        ...
                        if (skill != null && slv > 0) {
                            ...
                            if (skillID == Thief.Steal) {
                                ...
                                int hp = Math.min(Math.min(damageSum * level.X / 100, user.getCharacter().getCharacterStat().getMHP() / 2), mob.getMaxHP());
