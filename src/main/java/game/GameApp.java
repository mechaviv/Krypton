...
public class GameApp {
    ...
    private void initializeGameData() {
        ...
        // Load Quests
        time = System.currentTimeMillis();
        if (!QuestMan.getInstance().loadDemand()) {
            Logger.logError("Failed in loading Quest Demand information");
            System.exit(-2147467259);
        }
        if (!QuestMan.getInstance().loadAct()) {
            Logger.logError("Failed in loading Quest Act information");
            System.exit(-2147467259);
        }
        if (!QuestMan.getInstance().loadInitialQuiz()) {
            Logger.logError("Failed in loading Quest Initial Quiz information");
            System.exit(-2147467259);
        }
        Logger.logReport("Loaded Quest Info in " + ((System.currentTimeMillis() - time) / 1000.0) + " seconds.");
    ...
    public void broadcastWorld(OutPacket packet) {
        for (Channel channel : getChannels()) {
